...
if [ "$PKG" = "apt" ]; then
    ...
    apt-get install -y -qq build-essential git curl wget net-tools unbound dns-utils openssl >/dev/null 2>&1 || true
else
    yum install -y -q gcc make git curl wget net-tools unbound bind-utils openssl >/dev/null 2>&1 || true
fi
...
progress "Cleaning up old proxy installations..."
...
if systemctl is-active --quiet goproxy 2>/dev/null; then
    ...
    echo -e "  ${DIM}→ Go proxy removed${NC}"
fi

if systemctl is-active --quiet 3proxy 2>/dev/null; then
    systemctl stop 3proxy 2>/dev/null || true
    echo -e "  ${DIM}→ Existing 3proxy stopped${NC}"
fi
...
echo -e "  ${DIM}→ Binary: /usr/local/bin/3proxy ($(ls -lh /usr/local/bin/3proxy | awk '{print $5}'))${NC}"
